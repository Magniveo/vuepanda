let base = {
  ...
  checkUrl(url) {
    ...
    var strRegex =
      '^((https|http|ftp)://)?' + // (https或http或ftp):// 可有可None
      "(([\\w_!~*'()\\.&=+$%-]+: )?[\\w_!~*'()\\.&=+$%-]+@)?" + // ftp的user@  可有可None
  ...
  previewImg(src, httpUrl) {
    // 图片Preview，目前只支持Single图片Preview
  ...
  // backGroundUrl 后台url，如果后台url直接从后台下载，Other全部通过点击a标签下载
  ...
  // 1、idWithparentId这两个字段必须有
  // 2、树形tree需要注意IdWithparentId循环依赖的问题
  // 3、callback每次生成一新的Node的时回调的方法
  ...
  getTreeAllParent(id, data) {
    // 获取某个Node的所有父NodeConfig2020.11.01
    var nodes = [];
